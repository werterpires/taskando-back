...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string; kind: string; itemId: string }> }) {
  ...
  if (organization.ownerUserId !== context.user.id) return Response.json({ error: "Somente o Owner pode alterar itens de estrutura." }, { status: 403 });
  if (kind !== "department" && kind !== "team") return Response.json({ error: "Tipo de estrutura inválido." }, { status: 400 });
  ...
  if (payload.name !== undefined) { const name = payload.name.trim().replace(/\s+/g, " "); if (!name || name.length > 100) return Response.json({ error: "Informe um nome de até 100 caracteres." }, { status: 400 }); update.name = name; }
  if (payload.description !== undefined) { const description = payload.description.trim(); if (description.length > 500) return Response.json({ error: "Use descrição de até 500 caracteres." }, { status: 400 }); update.description = description; }
  ...
  try {
    if (payload.parentType !== undefined || payload.parentId !== undefined) {
      const parentType = payload.parentType ?? null; const parentId = payload.parentId ?? null;
      if (kind === "department" && parentType !== null && parentType !== "organization") return Response.json({ error: "Departamento só pode ficar na Organização ou na Área pessoal." }, { status: 400 });
      if (kind === "team" && parentType !== null && parentType !== "organization" && parentType !== "department") return Response.json({ error: "Time só pode ficar na Organização, em Departamento ou na Área pessoal." }, { status: 400 });
      if (parentType === "organization") {
        const allowed = await canAccessOrganization(context.db, context.user.id, context.user.email, parentId!, "edit");
        if (!allowed) return Response.json({ error: "Você não pode criar itens na organização de destino." }, { status: 403 });
        update.organizationId = parentId!;
      }
    ...
    const [item] = await context.db.update(table).set(update).where(and(eq(table.id, itemId), eq(table.organizationId, id))).returning();
    if (!item) return Response.json({ error: "Item não encontrado." }, { status: 404 });
    if (payload.parentType !== undefined || payload.parentId !== undefined) {
      const parentType = payload.parentType ?? null; const parentId = payload.parentId ?? null;
      await context.db.insert(hierarchyAttachments).values({ id: crypto.randomUUID(), childType: kind, childId: itemId, parentType, parentId, updatedAt: new Date().toISOString() }).onConflictDoUpdate({ target: [hierarchyAttachments.childType, hierarchyAttachments.childId], set: { parentType, parentId, updatedAt: new Date().toISOString() } });
    }
